...
var jsonTree = (function() {
     ...
     * @param isLast {boolean} - true if node is last in list of parent childNodes
     */
    function NodeUint8Array(label, val, isLast) {
        this.type = "Uint8Array";
        
        // Hex
        //_NodeSimple.call(this, label, '' + (function(val) {
        //    return val.reduce(function(memo, i) {return memo + ('0' + i.toString(16)).slice(-2)}, '')
        //})(val) + '', isLast);

        // Base64
        _NodeSimple.call(this, label, 'Uint8Array( ' + window.btoa(String.fromCharCode.apply(null, val)).replace(/\+/g, '-').replace(/\//g, '_').replace(/=*$/, '') + ' )[' + val.length + ']', isLast);
    }
    utils.inherits(NodeUint8Array,_NodeSimple);
     ...
     * @param isLast {boolean} - true if node is last in list of parent childNodes
     */
    function NodeArrayBuffer(label, val, isLast) {
        ...
        _NodeSimple.call(this, label, 'ArrayBuffer( ' + window.btoa(String.fromCharCode.apply(null, val)).replace(/\+/g, '-').replace(/\//g, '_').replace(/=*$/, '') + ' )[' + val.length + ']', isLast);
    }
    utils.inherits(NodeArrayBuffer,_NodeSimple);
